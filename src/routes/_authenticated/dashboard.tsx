...
import { LogOut, ShieldCheck, Users, DollarSign, Calendar, FileText, Eye, Search } from "lucide-react";
...
function Dashboard() {
  ...
  const { data, isLoading, error } = useQuery({ queryKey: ["account"], queryFn: () => fetchAccount({}) });

  // Only enable admin queries if admin status is confirmed and data is loaded
  const isAdmin = data?.isAdmin === true || signedInEmail === CONFIGURED_ADMIN_EMAIL;
  const payments = useQuery({ queryKey: ["admin-payments"], queryFn: () => fetchPayments({}), enabled: isAdmin && !!data });
  const users = useQuery({ queryKey: ["admin-users"], queryFn: () => fetchUsers({}), enabled: isAdmin && !!data });
  const userDetails = useQuery({
    queryKey: ["admin-user-details", selectedUserId],
    queryFn: () => fetchUserDetails({ data: { userId: selectedUserId! } }),
    enabled: !!selectedUserId && isAdmin && !!data,
  });
    ...
    <PageShell>
      ...
      {error && (
        <Card className="mx-auto max-w-md text-center">
          <h2 className="text-lg font-bold text-red-700">خطا در بارگذاری</h2>
          <p className="mt-2 text-sm text-ink/70">
            {error instanceof Error ? error.message : "خطایی رخ داد."}
          </p>
          <button
            onClick={() => window.location.reload()}
            className="mt-4 rounded-full bg-ink px-4 py-2 text-sm font-bold text-cream"
          >
            تلاش مجدد
          </button>
        </Card>
        ...
        <>
          ...
          {isAdmin && activeTab === "payments" && (
            <Card className="lg:col-span-2">
              ...
              {payments.isLoading ? (
                <p className="mt-3 text-[13px] text-ink/60">در حال بارگذاری…</p>
                  ...
                  {filteredUsers.map((user) => (
                    ...
                    >
                      ...
                      <div className="mt-2 flex items-center gap-4 text-[12px] text-ink/60">
                        ...
                        <span>پرداخت‌ها: {user.payments?.length || 0}</span>
                      </div>
                    </div>
                  ))}
                </div>
